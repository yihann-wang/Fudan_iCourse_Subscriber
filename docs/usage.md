...
如果提示“录像未下载完整”或“提取出的音频不完整”，故障发生在本地读取录像阶段，还未上传本次音频，与语音 API Key 无关。文件头可能写着完整课时，但实际视频只保存了一部分，所以播放器显示的总时长不能证明下载完整。

- 0.6.3 起会检查 MP4 数据块是否超出文件结尾；已被旧版标为完成的残缺 MP4 也不会继续复用。此检查只读取少量文件头，不会逐帧扫描所有课程，也不能替代完整解码检查。
- 0.6.6 起优先按 32 MiB 请求录像；服务器提供可靠文件标识时，下载中断会保留断点，下次续传。日志会显示“继续下载 · 已保留 … MB”。文件在服务器上发生变化时会重新下载；不支持范围请求或安全续传标识的服务器会使用完整下载。
- 断点文件放在录像目录下隐藏的 `.icourse` 中，成功后自动清理；不要在重试前删除它。记录不含带签名地址或登录凭据。旧版已删除的临时下载无法恢复，旧残缺 MP4 没有对应的续传标识，也不会直接拼接。
- 上次使用“下载并生成笔记”时，可直接“仅重试失败课次”，检测到残缺录像会先重新下载；已成功的课次保持不变。
- 上次使用本地处理模式时，改选“下载并生成笔记”，填写出错课程及课次 ID，保持原保存位置，再点“开始任务”。本地模式不能从学校重新下载。
- 旧录像会保留到替代文件通过检查后再被替换，不需要先删除整个课程目录。如果重新下载仍失败，请检查学校网页回放能否播放至结尾，并检查网络和外置盘连接；仅凭此错误无法确定是服务器、传输还是存储出了问题。
