...
| 语音服务 | 兼容语音服务，或阿里云百炼（支持同步字幕） |
| 百炼语音模型、API Key | 选择 `fun-asr` 或 `paraformer-v2`，填写自己的百炼密钥；与兼容服务密钥分开保存 |
...
需要同步字幕时，0.6.0 可选择阿里云百炼的 **Fun-ASR** 或 **Paraformer**，自动生成与 MP4 同名的 SRT。旧课需勾选“重新生成已有转录和笔记”才能用新模型补字幕，录像继续复用。[百炼设置及验证范围](docs/cloud-asr.md#阿里云百炼fun-asr--paraformer)
